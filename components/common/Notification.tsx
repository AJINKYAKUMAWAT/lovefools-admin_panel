...
const Notification = () => {
  ...
  const typedVariant = variant as NotificationVariant;

  const handleClose = () => {
    dispatch(hideNotification());
  };

  const colorVariants = (variant: NotificationVariant): string => {
    switch (variant) {
      case 'success':
        return 'bg-green-600';
      case 'error':
  ...
  const iconVariants = (variant: NotificationVariant): JSX.Element => {
    switch (variant) {
      case 'success':
  ...
  const Icon: JSX.Element = iconVariants(typedVariant);
